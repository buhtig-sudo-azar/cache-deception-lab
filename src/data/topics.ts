...
Веб-кэширование тесно связано с HTTP-протоколом: заголовки Cache-Control, Vary, Age управляют поведением кэша. Load Balancers часто совмещены с кэшем. WAF (Web Application Firewall) может влиять на кэширование. Понимание кэша необходимо для изучения Web Cache Deception и Cache Poisoning.`,
        ...
        diagram: {
          ...
          input: { label: 'HTTP-запрос', url: 'GET /resource' },
          leftBranch: {
            label: 'Кэш (CDN/Proxy)',
            subtitle: 'Проверка наличия',
            steps: [
              { label: 'Формирование ключа', detail: 'Метод + Хост + Путь', role: 'process' },
              { label: 'Поиск в кэше', detail: 'Ключ найден?', role: 'decision' },
              { label: 'Cache HIT', detail: 'Мгновенный ответ', role: 'success' },
            ],
          },
          rightBranch: {
            label: 'Origin Server',
            subtitle: 'Обработка запроса',
            steps: [
              { label: 'Проброс запроса', detail: 'Cache MISS → серверу', role: 'process' },
              { label: 'Генерация ответа', detail: 'Динамический контент', role: 'process' },
              { label: 'Cache MISS', detail: 'Ответ сохранён в кэш', role: 'warning' },
            ],
          },
          conclusion: { left: 'HIT: Мгновенно', right: 'MISS: С задержкой', highlight: 'Кэш не проверяет содержимое!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
Неправильные заголовки — корневая причина WCD. Если бы все серверы правильно использовали Cache-Control: no-store для динамического контента, WCD не существовал бы. Cache Keys и Cache Rules опираются на эти заголовки.`,
        ...
        diagram: {
          ...
          steps: [
            { label: 'Получен ответ сервера', role: 'input' },
            { label: 'Проверка Cache-Control', detail: 'Разбор директив', role: 'decision' },
            { label: 'no-store → НЕ кэшировать', detail: 'Абсолютный запрет', role: 'danger', branchLabel: 'no-store' },
            { label: 'no-cache → Проверять каждый раз', detail: 'Хранить, но валидировать', role: 'warning', branchLabel: 'no-cache' },
            { label: 'max-age=N → Кэшировать на N сек', detail: 'Хранить N секунд', role: 'success', branchLabel: 'max-age' },
            { label: 'Нет заголовка? Эвристика', detail: '10% от Last-Modified', role: 'neutral' },
          ],
        } as import('@/types').ProcessingFlowDiagramData,
        practicalExamples: [
          {
...
Vary-заголовок расширяет ключ. Vary: Cookie добавляет значение Cookie к ключу. Vary: Accept-Encoding создаёт разные записи для gzip/br/none.`,
        ...
        diagram: {
          type: 'split-flow',
          title: 'Формирование Cache Key: что входит, а что нет',
          input: { label: 'HTTP-запрос', url: 'GET /profile HTTP/1.1' },
          leftBranch: {
            label: 'Входит в ключ',
            subtitle: 'Keyed Components',
            steps: [
              { label: 'Метод: GET', detail: 'Только GET кэшируется', role: 'success' },
              { label: 'Хост: example.com', detail: 'Разные домены = разные ключи', role: 'success' },
              { label: 'Путь: /profile', detail: 'Основной компонент ключа', role: 'success' },
            ],
          },
          rightBranch: {
            label: 'НЕ входит в ключ',
            subtitle: 'Unkeyed Components',
            steps: [
              { label: 'Cookie: session=abc', detail: 'Только если Vary: Cookie', role: 'danger' },
              { label: 'Authorization: Bearer ...', detail: 'Игнорируется кэшем', role: 'danger' },
              { label: 'Фрагмент: #section', detail: 'Не отправляется на сервер', role: 'neutral' },
            ],
          },
          conclusion: { left: 'GET|example.com|/profile', right: 'Cookie НЕ в ключе!', highlight: 'Один ключ = один ответ для всех!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
Если Cache-Control: no-store, расширение не имеет значения. Но если сервер не отправляет этот заголовок — расширение побеждает.`,
        ...
        diagram: {
          type: 'split-flow',
          title: 'Static Extension Rule: кэш vs сервер',
          input: { label: 'URL запроса', url: '/profile.css' },
          leftBranch: {
            label: 'Кэш',
            subtitle: 'Проверка расширения',
            steps: [
              { label: 'Видит расширение .css', detail: 'В списке статических!', role: 'process' },
              { label: 'Правило: кэшировать', detail: 'Статический файл', role: 'success' },
              { label: 'Сохраняет ответ', detail: 'Профиль как CSS', role: 'danger' },
            ],
          },
          rightBranch: {
            label: 'Origin Server',
            ...
            steps: [
              { label: 'Игнорирует .css', detail: 'Мапит на /profile', role: 'process' },
              { label: 'Генерирует профиль', detail: 'Динамический HTML', role: 'process' },
              { label: 'Нет Cache-Control!', detail: 'no-store отсутствует', role: 'danger' },
            ],
          },
          conclusion: { left: 'Кэш: сохранено!', right: 'Сервер: профиль!', highlight: 'УЯЗВИМОСТЬ: Профиль в кэше!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
Директория имеет более низкий приоритет, чем расширение. Но если нет расширения, правило директории вступает в силу.`,
        ...
        diagram: {
          ...
          input: { label: 'URL запроса', url: '/static/../../profile' },
          leftBranch: {
            label: 'Кэш',
            subtitle: 'Проверка директории',
            steps: [
              { label: 'Начинается с /static/', detail: 'Проверка префикса', role: 'process' },
              { label: 'НЕ нормализует ../', detail: 'Путь как есть', role: 'process' },
              { label: 'Правило: кэшировать', detail: 'Статическая директория', role: 'success' },
            ],
          },
          rightBranch: {
            label: 'Origin Server',
            subtitle: 'Нормализация пути',
            steps: [
              { label: 'Декодирует путь', detail: 'Нормализация', role: 'process' },
              { label: '/static/../../profile', detail: '→ /profile', role: 'process' },
              { label: 'Динамический профиль!', detail: 'Приватные данные', role: 'danger' },
            ],
          },
          conclusion: { left: 'Кэш: /static/* = кэшировать', right: 'Сервер: /profile = динамический', highlight: 'PATH TRAVERSAL ОБХОД!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
Главное отличие: в Deception атакующий получает данные жертвы, а в Poisoning жертва получает вредоносный контент атакующего.`,
        ...
        diagram: {
          type: 'cause-effect',
          title: 'Сценарий WCD-атаки: цепочка причин и следствий',
          steps: [
            { label: 'Атакующий: /profile.css', detail: 'Свой профиль + расширение .css', role: 'cause' },
            { label: 'Кэш видит .css → кэширует', detail: 'Расширение статическое', role: 'effect' },
            { label: 'Сервер игнорирует .css', detail: 'Возвращает /profile', role: 'effect' },
            { label: 'Кэш сохраняет профиль', detail: 'Приватные данные в кэше', role: 'danger' },
            { label: 'Жертва: /profile.css → HIT', detail: 'Получает чужой профиль!', role: 'danger' },
          ],
        } as import('@/types').CauseEffectData,
        practicalExamples: [
          {
...
**%3F (?)** — Кэш декодирует, сервер не декодирует. Риск: **средний** — кодированный вопросительный знак может обойти фильтры.`,
        ...
        diagram: {
          ...
          leftBranch: {
            label: 'Кэш',
            subtitle: 'Парсинг URL',
            steps: [
              { label: 'Видит расширение .css', detail: 'После разделителя ;', role: 'process' },
              { label: '.css в списке статических', detail: 'Правило: кэшировать', role: 'success' },
              { label: 'Сохраняет ответ', detail: 'Профиль как CSS', role: 'danger' },
            ],
          },
          rightBranch: {
            ...
            subtitle: 'Обработка запроса',
            steps: [
              { label: 'Игнорирует ;.css', detail: 'Path parameter отсечён', role: 'process' },
              { label: 'Маршрут: /profile', detail: 'Динамический контент', role: 'process' },
              { label: 'Возвращает профиль', detail: 'Персональные данные', role: 'danger' },
            ],
          },
          conclusion: { left: 'Кэш: .css = кэшировать', right: 'Сервер: /profile = данные', highlight: 'DISCREPANCY: Профиль в кэше!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
Это означает: сначала ищи файл, потом директорию, потом перенаправь на index.php. Кэш не знает про try_files и судит только по расширению.`,
        ...
        diagram: {
          type: 'split-flow',
          title: 'Path Mapping: кэш vs сервер (try_files fallback)',
          input: { label: 'URL запроса', url: '/profile.css' },
          leftBranch: {
            label: 'Кэш',
            subtitle: 'Буквальный маппинг',
            steps: [
              { label: 'Расширение .css', detail: 'Статический файл', role: 'process' },
              { label: 'Ключ: /profile.css', detail: 'Литеральный путь', role: 'process' },
              { label: 'КЭШИРОВАТЬ', detail: 'Статический ресурс', role: 'success' },
            ],
          },
          rightBranch: {
            label: 'Сервер (Nginx)',
            subtitle: 'try_files fallback',
            steps: [
              { label: 'Файл profile.css?', detail: 'Не найден!', role: 'decision' },
              { label: 'Fallback: /profile', detail: 'Обработчик профиля', role: 'process' },
              { label: 'Динамический контент', detail: 'Приватные данные', role: 'danger' },
            ],
          },
          conclusion: { left: 'Кэш: сохранено как .css', right: 'Сервер: профиль из fallback', highlight: 'FALLBACK DISCREPANCY!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
**/a//b** — Кэш различает двойные слеши, сервер схлопывает в /a/b. Риск: **низкий** — большинство современных серверов нормализуют //.`,
        ...
        diagram: {
          type: 'split-flow',
          title: 'Normalization: кэш vs сервер',
          input: { label: 'URL запроса', url: '/static/..%2Fprofile' },
          leftBranch: {
            label: 'Кэш',
            subtitle: 'Без нормализации',
            steps: [
              { label: 'Не декодирует %2F', detail: 'Путь как есть', role: 'process' },
              { label: 'Видит: /static/..%2Fprofile', detail: 'Начинается с /static/', role: 'process' },
              { label: 'Правило: кэшировать', detail: 'Статическая директория', role: 'success' },
            ],
          },
          rightBranch: {
            label: 'Origin Server',
            subtitle: 'С нормализацией',
            steps: [
              { label: 'Декодирует %2F → /', detail: '/static/../profile', role: 'process' },
              { label: 'Нормализует ../', detail: '→ /profile', role: 'process' },
              { label: 'Динамический профиль!', detail: 'Приватные данные', role: 'danger' },
            ],
          },
          conclusion: { left: 'Кэш: /static/* = кэшировать', right: 'Сервер: /profile = данные', highlight: 'NORMALIZATION DISCREPANCY!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
...
**Varnish** — полный контроль через VCL. Администратор сам определяет, какие расширения кэшировать. Самый гибкий, но и самый сложный подход.`,
        ...
        diagram: {
          ...
          input: { label: 'URL запроса', url: '/profile.css.php' },
          leftBranch: {
            label: 'Nginx',
            subtitle: 'Первое расширение',
            steps: [
              { label: 'Видит: .css (первое)', detail: 'profile.css.php', role: 'process' },
              { label: '.css в списке статических', detail: 'Правило: кэшировать', role: 'success' },
              { label: 'Сохраняет ответ!', detail: 'Профиль в кэше', role: 'danger' },
            ],
          },
          rightBranch: {
            label: 'Cloudflare',
            subtitle: 'Последнее расширение',
            steps: [
              { label: 'Видит: .php (последнее)', detail: 'profile.css.php', role: 'process' },
              { label: '.php НЕ статическое', detail: 'Правило: пропуск', role: 'warning' },
              { label: 'НЕ кэширует', detail: 'Запрос идёт к серверу', role: 'success' },
            ],
          },
          conclusion: { left: 'Nginx: .css = КЭШИРОВАТЬ', right: 'CF: .php = ПРОПУСК', highlight: 'Результат зависит от кэша!' },
        } as import('@/types').SplitFlowDiagramData,
        practicalExamples: [
          {
        ...
        diagram: {
          ...
          root: {
            ...
            children: [
              ...
              {
                label: 'CDN',
                detail: 'Правила кэширования',
                children: [
                  { label: 'Не кэшировать /api/*' },
                  { label: 'Нормализация ключей' },
                ],
              },
              {
                label: 'Архитектура',
                ...
                children: [
                  { label: 'Разные домены' },
                  { label: 'Разделение static/dynamic' },
                ],
              },
              {
                label: 'Мониторинг',
                detail: 'Обнаружение',
                children: [
                  { label: 'X-Cache: HIT на API' },
                  { label: 'Алерты на утечки' },
                ],
              },
            ],
          },
        } as import('@/types').DependencyGraphData,
        practicalExamples: [
          {
